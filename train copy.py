...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, comp):
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        gt_image = latent_image_dataset.imgs_dict[viewpoint_cam.image_name]
        # print(image.shape)
        # print(gt_image.shape)
        Ll1 = l1_loss(image, gt_image.float())
        ...
        with torch.no_grad():
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, vae, latent_image_dataset, render, (pipe, background))
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    # latent_image = renderFunc(viewpoint, scene.gaussians, *renderArgs)["render"][None]
                    # image = torch.clamp((vae.decode(latent_image, return_dict=False)[0] + 1) / 2, 0.0, 1.0)
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs)["render"], 0.0, 1.0)
                    gt_image = torch.clamp(latent_image_dataset.imgs[idx].to("cuda"), 0.0, 1.0)
